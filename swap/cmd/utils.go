...
func checkParameterWithoutPassW(client *rpc.Client, fromHex, toHex, amountStr, feeStr string, nonceNum uint64, payload []byte) (*types.TransactionData, error) {
	info := &types.TransactionData{}
	var err error
	if len(toHex) > 0 {
		toAddr, err := common.HexToAddress(toHex)
		if err != nil {
			return info, fmt.Errorf("invalid receiver address: %s", err)
		}
		info.To = toAddr
	...
	if len(fromHex) > 0 {
		fromAddr, err := common.HexToAddress(fromHex)
		if err != nil {
			return info, fmt.Errorf("invalid receiver address: %s", err)
		}
		info.From = fromAddr
	...
	if client != nil {
		...
		if nonceNum == nonce || nonceNum == DefaultNonce {
			info.AccountNonce = nonce
		} else {
			if nonceNum < nonce {
				return info, fmt.Errorf("your nonce is: %d,current nonce is: %d,you must set your nonce greater than or equal to current nonce", nonceValue, nonce)
			}
			info.AccountNonce = nonceNum
		}
